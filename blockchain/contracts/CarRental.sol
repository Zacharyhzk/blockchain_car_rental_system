...
contract CarRental is Ownable {
   ...
   // Allow ABC Company to add car information
    ...
    payable public onlyOwner infoCorrect(carType) {
    ...
    function confirmReturn(string memory carPlate, uint damageCharge, uint returnYear, uint returnMonth, uint returnDay) 
        ...
        address payable _walletAdd = payable(records[recordIndex].walletAddress);
        ...
        contractBalance = contractBalance + 5 ether;
    ...
    payable public {
        ...
        address payable _walletAdd = payable(walletAdd);
